...
# 4. Diagnostic DB
echo "[DIAG] DATABASE_URL dans backend : $(docker compose exec -T backend printenv DATABASE_URL)"
echo "[DIAG] Test connexion psql..."
docker compose exec -T db psql -U user -d crm -c "SELECT 1" 2>&1 || echo "[DIAG] psql échoué"
